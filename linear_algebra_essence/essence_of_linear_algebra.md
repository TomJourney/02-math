...
1）虽然我们都关注空间中的同一个向量，在标准坐标系使用$\begin{bmatrix} 3 \\ 2\end{bmatrix}$来表示，但在基向量$b_1=\begin{bmatrix}2 \\ 1\end{bmatrix}, b_2=\begin{bmatrix}-1 \\ 1\end{bmatrix}$定义的坐标系中，使用$\begin{bmatrix} 3/5 \\ 1/5 \end{bmatrix}$ 来表示。
...
1）在基向量$b_1=\begin{bmatrix}2 \\ 1\end{bmatrix}, b_2=\begin{bmatrix}-1 \\ 1\end{bmatrix}$定义的坐标系中，使用坐标(-1, 2)来描述一个向量，那么这个向量的坐标计算如下。
$$
...
1）例：在我们的坐标系，即标准坐标系中，有一个坐标为(3,2)的向量；我如何计算出詹尼佛的坐标系中的坐标为(5/3,1/3)呢？
...
2）在这里，<font color=red>对于以詹尼佛的基向量作为列的基变换矩阵</font>，其逆矩阵的两列为(1/3, -1/3), (1/3, 2/3)；如果想知道向量$\begin{bmatrix}3 \\ 2\end{bmatrix}$在詹尼佛的坐标系如何表示，我们用这个基变换矩阵的逆乘以向量$\begin{bmatrix}3 \\ 2\end{bmatrix}$，结果为(5/3, 1/3)。
