...
1）虽然我们都关注空间中的同一个向量，在标准坐标系使用坐标$\begin{bmatrix} 3 \\ 2\end{bmatrix}$来表示，但在基向量$b_1=\begin{bmatrix}2 \\ 1\end{bmatrix}, b_2=\begin{bmatrix}-1 \\ 1\end{bmatrix}$定义的坐标系中，使用坐标$\begin{bmatrix} 3/5 \\ 1/5 \end{bmatrix}$ 来表示。（<font color=red>同一个物理点在不同坐标系的坐标是不同的</font>）
...
<font color=red>这些在变换后被拉伸n倍的向量就被称为特征向量</font>；
...
- 相比之下，尝试计算一个非对角矩阵的100次幂，非常难计算，这就是一场噩梦；
...
2）用特征向量来完成这件事情的意义在于：这个新矩阵必然是对角的，并且对角元为对应的特征值；
...
1）定义：<font color=red>一组基向量（同样是特征向量）所构成的集合，被称为一组特征基，这也非常合理</font>；
...
- 一种更容易的做法是：先变换到特征基，在那个坐标系中计算100次幂，然后转换会标准坐标系；
