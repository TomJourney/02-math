...
- 你可能会尝试只将旋转矩阵的列转换为用詹尼佛的语言来描述；
- 但是并不尽然，这些列代表的是$\hat{i}, \hat{j}$的去向；
- 但是詹尼佛想要的矩阵需要代表它的基向量的去向，并且使用它的语言来描述；
