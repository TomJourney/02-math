...
这与基向量$\hat{i}=\begin{bmatrix}1 \\ 0\end{bmatrix}, \hat{j}=\begin{bmatrix} 0 \\ 1\end{bmatrix}$ 定义的坐标系不是同一个坐标系，可以理解为是不同语言或在不同坐标系来描述同一个向量。
...
3）从几何上说，这个矩阵是将我们的标准网格变换为詹尼佛的网格；但是从数值上说，这是用詹尼佛的语言描述转换为用我们的语言描述；
