...
- 更重要的是，这个函数是线性的，因为它顺利通过了线性检验，即直线上等距分布的点在投影到数轴后仍然等距分布；
...
<img src="./img/ch07_10.png" width="50%" />
...
<img src="./img/ch07_11.png" width="50%"/>
...
<font color=red>这就是为什么给定向量$\vec{v}$与单位向量$\vec{u}$的点积可以解读为：将给定向量$\vec{v}$投影到单位向量$\vec{u}$所在的直线上所得到的投影长度</font>；

<img src="./img/ch07_15.png" width="50%"/>

【投影结果】

<img src="./img/ch07_15_result.png" width="50%"/>
...
<img src="./img/ch07_16.png" width="50%"/>
...
1）<font color=red>这里给你的启发是：你在任何时候看到一个线性变换，它的输出空间是一维数轴，无论它是如何定义的，空间中会存在唯一的向量v与该线性变换相关</font>；(这个线性变换是一个1X n的线性变换矩阵，其基向量是$\vec{u}$，把基向量$\vec{u}$倾倒下来就是一个1X n的线性变换矩阵)
...
<img src="./img/ch07_20.png" width="60%"/>
...
<img src="./img/ch07_21.png" width="60%"/>
...
1）叉积定义：$\vec{v}$与$\vec{w}$的叉积等于这2个向量围成的平行四边形的面积；

- 若v在w右侧，则叉积为正，且值等于平行四边形的面积；
- 若v在w左侧，则叉积为负，且值等于平行四边形的面积的相反数；
- 显然，叉积的向量顺序会影响叉积正负；

$$
\vec{v} \times \vec{w}=2个向量围成的平行四边形的面积
$$



 <img src="./img/ch0801_01.png" width="60%" align="center"/>
...
<img src="./img/ch0801_02.png" width="60%" />

1）为什么v与w叉积的结果可以通过v与w组成的矩阵的行列式计算出来？

- 因为由v和w为列的坐标所构成的矩阵与一个将$\hat{i}$与$\vec{j}$分别移动到v和w的线性变换相对应；
- 而行列式就是变换前后面积变化比例的度量；
- 而我们关注的：就是以$\hat{i}$与$\hat{j}$为边的单位正方形；在变换后，这个单位正方形变换为我们关心的平行四边形；

2）所以说，通常用来度量面积变化比例的行列式，在这里给出了平行四边形的面积；因为这个平行四边形来自于面积为1的正方形；更重要的是，如果v在w的左边，说明变换后定向发生了变化，所以行列式为负；

<img src="./img/ch0801_03.png" width="60%"/>

【例】向量v与w的叉积通过行列式计算；

<img src="./img/ch0801_04.png" width="60%"/>

3）另一个发现：相互垂直或接近垂直的2个向量的叉积比快要接近的2个向量的叉积更大；

- 因为当两条边接近垂直时，平行四边形的面积更大；
...
1）真正的叉积：通过两个三维向量生成一个新的三维向量（<font color=red>即叉积的结果是一个向量）</font>；

2）与之前一样：我们还是要考虑这两个向量围成的平行四边形；

- 说的具体一些，如这2个向量围成的平行四边形的面积为2.5；但叉积的结果不是一个数，而是一个向量；
- 叉积产生的向量的长度就是平行四边形面积2.5；且这个向量的方向与平行四边形垂直；
- 不过是哪条边呢？因为长度为2.5，并且垂直于给定面的向量有两个；
  - <font color=red>这时就需要用到右手定则</font>；即右手食指指向v的方向，伸出中指指向w的方向，当你把大拇指竖起来时，大拇指的方向就是叉积方向；

<img src="./img/ch0801_06.png" width="60%"/>
...
1）$\vec{v}=\begin{bmatrix}0 \\ 0\\2\end{bmatrix}$ ，$\vec{w}=\begin{bmatrix} 0 \\ 2 \\ 0\end{bmatrix}$，显然v指向z轴正方向，w指向y轴正方向；它们围成的图形实际上是一个正方形；

- 所以，$\vec{v} \times \vec{w}$的叉积是一个长度为4的向量，这个向量与v和w围成的正方形垂直；
- 根据右手定则，它们的叉积向量应该指向x轴负方向；
- 因此，$\vec{v} \times \vec{w}$的叉积是-4乘以$\hat{i}$；

<img src="./img/ch0801_07.png" width="60%"/>
...
$$
\vec{v} \times \vec{w}=\begin{bmatrix}v_1 \\ v_2 \\ v_3\end{bmatrix} \times \begin{bmatrix}w_1 \\ w_2 \\ w_3\end{bmatrix}=det(\begin{bmatrix} \hat{i} &v_1 &w_1 \\ \hat{j} &v_2 &w_2 \\ \hat{k} &v_2 &w_2\end{bmatrix})
$$

<img src="./img/ch0801_08.png" width="70%"/>

【图解】$\vec{v} \times \vec{w}$得到的是这3个基向量（$\hat{i}, \hat{j}, \hat{k}$）的线性组合。

1）<font color=red>这3个基向量的上述线性组合得到的向量（叉积产生的向量），是唯一一个与v和w组成平面垂直，长度为v和w围成的平行四边形面积，并且遵循有右手定则的向量</font>；

2）当然，从某种意义上讲，这就是符号上的技巧，但这么做是有原因的； 

- 行列式重要性的体现并非完全巧合；
- 基向量作为矩阵元也不是信手而为；
- 要理解这样表示的源头，我们需要借助上一期讲的对偶性； 
...
1）每当你看到一个从空间到数轴的线性变换，你都能找到一个向量，<font color=red>被称为这个变换的对偶向量</font>；

- 使得应用线性变换和与对偶向量点乘等价；

<img src="./img/ch0802_02.png" width="60%"/>

<br>

---

2）作者要做的是定义一个从三维空间到数轴的特定变换；

- 并且这个变换是根据向量v与w来定义的；
- 然后当我们把这个变换与三维空间中的对偶向量关联时，这个对偶向量就是向量v与w的叉积，即$\vec{v} \times \vec{w}$
- <font color=red>之所以这么做，是因为理解这个变换，能够解释清楚叉积的计算过程和几何含义之间的关系</font>；

<img src="./img/ch0802_03.png" width="60%"/>

<br>

---

## 【8-2.1】如何在二维空间中计算向量叉积？

- 你有两个向量v与w，v作为矩阵第一列，w作为矩阵第二列，然后计算矩阵行列式的结果，就是叉积的值；
- 几何上说，该矩阵行列式给出了两个向量张成的平行四边形的面积；
- 它还可能出现负值，取决于这两个向量的定向；

<img src="./img/ch0802_04.png" width="60%"/>
...
1）如果你并不知道三维向量叉积如何计算，并且尝试取推导；

- 你可能会想到，它涉及3个向量，u，v，w；
- 把这3个向量的坐标作为3X3矩阵的列，然后计算这个矩阵的行列式；
- 从几何上讲，这个行列式给出了三个向量张成的平行六面体的体积；（外加一个正负号，取决于这三个向量是否满足右手定则）
- 结论：实际上，这并不是三维向量的真正叉积；

<img src="./img/ch0802_05.png" width="60%"/>

2）<font color=red>真正三维向量的叉积：接收两个向量，输出一个向量</font>(而并不是接收3个向量并输出一个数，不过该想法已经非常接近真实叉积了)；

<img src="./img/ch0802_07.png" width="60%"/>

3）关于三维空间计算向量叉积的想法：
$$
\vec{u} \times \vec{v} \times \vec{w}=det(\begin{bmatrix}u_1 & v_1 & w_1 \\ u_2 & v_2 & w_2 \\ u_3 & v_3 & w_3 \end{bmatrix})
$$
将第1个向量u看做可变向量，比如(x,y,z)，而v与w保持不变；如下：
$$
\vec{u} \times \vec{v} \times \vec{w}=det(\begin{bmatrix}x & v_1 & w_1 \\ y & v_2 & w_2 \\ z & v_3 & w_3 \end{bmatrix})
$$
4）那么我们就有一个从三维空间到数轴的函数了；

- 你输入一个向量(x,y,z)，然后通过矩阵行列式得到一个数；这个向量的第一列是(x,y,z)，其余两列是常向量v和w的坐标；
- <font color=red>这个函数的几何意义：对于任意一个输入向量(x,y,z)，你都考虑由它和v与w确定的平行六面体，得到该六面体的体积并根据定向确定符号</font>；这个函数对于叉积很关键；
- 这个函数的一个很重要的性质是它是线性的；作者让学习者自行证明；
- <font color=red>一旦该函数是线性的，我们就可以开始引进对偶性的思想；即我们可以通过矩阵乘法来描述这个函数</font>；
  - 具体说，因为这个函数从三维空间到一维空间，就会存在一个1X3矩阵来代表这个变换；

<img src="./img/ch0802_08.png" width="50%"/>

5）对偶性整体思路：从三维空间到一维空间的变换的特别之处：

- 在于你可以将这个矩阵立起来，并且把整个变换看做与这个特定向量的点积；

<img src="./img/ch0802_09.png" width="50%" />

6）我们要找的就是这个特殊的三维向量，我们称之为向量$\vec{p}$

- 使得p与其他任意向量(x,y,z)的点积等于一个3X3矩阵的行列式；这个矩阵的第1列为(x,y,z)，其余两列为v和w的坐标；

<img src="./img/ch0802_10.png" width="60%"/>

<p align="center">【图ch0802_10】</p>
...
$$
等式左边执行点积运算，右边执行行列式运算，结果如下：

<img src="./img/ch0802_11.png" width="60%"/>
...
等号右侧的计算过程，对于那些进行过叉积计算的人来说是很熟悉的；

像这样，合并x，y，z前面的的常数项，和把$\vec{i}$,$\vec{j}$,$\vec{k}$ ，放进矩阵第一列进行计算，然后合并各项前面的系数没有区别；

<img src="./img/ch0802_13.png" width="60%"/>

在矩阵中插入$\hat{i},  \hat{j},  \hat{k}$， 不过是在传递一个信号，告诉我们应该把这些系数解读为一个向量的坐标，即把$v_2 \cdot w_3 - v_3 \cdot w_2,  v_3 \cdot w_1 - v_1 \cdot w_3 ,  v_1 \cdot w_2- v_2\cdot w_1$作为向量坐标；

9）小结：因此，这一切都在说明，这个奇怪的运算过程可以看做是下面问题的答案；

- 当你将向量p和某个向量(x,y,z)点乘时，所得结果等于3X3矩阵的行列式；这个矩阵第一列为(x,y,z)，其余两列为v和w的坐标；
- 什么样的向量p才能满足这一特殊性质呢？
...
## 【8-2.3】从几何角度回答-什么样的向量p才能满足这一特殊性质

1）问题：当你将向量p和某个向量(x,y,z)点乘时，所得结果等于一个由(x,y,z)和v与w确定的平行六面体的有向体积；什么样的向量p才能满足这一特殊性质呢？

<img src="./img/ch0802_14.png" width="60%"/>

2）向量p与其他向量点积的几何解释：是将其他向量投影到p上，然后将投影长度与p长度相乘；

3）考虑到这一点，对于我们所关心的平行六面体体积，我来说明一种思考方法：

- 首先获得由v和w确定的平行四边形的面积；
- 乘以向量(x,y,z) 在垂直于平行四边形方向上的分量（不是(x,y,z)的长度）；

4）换句话说，我们找到的线性函数对于给定向量的作用，是将这个向量投影到垂直于v与w的直线上； 然后将投影长度与v和w张成的平行四边形的面积相乘；

<img src="./img/ch0802_15.png" width="60%"/>

但是，这和垂直于v和w且长度为平行四边形面积的向量与(x,y,z)点乘是同一回事情；

5）更重要的是，如果你选择了合适的向量方向，点积为正的情况就会与(x,y,z), v和w满足右手定则的情况相吻合；

- 这意味着，我们找到了一个向量p，使得p与某个向量(x,y,z)点乘时，所得结果等于一个3X3矩阵的行列式；这个矩阵的三列分别是(x,y,z)，v的坐标，w的坐标；

因此我们之前通过特殊符号技巧进行计算所得到的向量，必然在几何上与这个向量对应； 这就是叉积的计算过程与几何解释有关联的根本原因； 

<img src="./img/ch0802_17.png" width="60%"/>
...
1）我们首先定义了一个三维空间到数轴的线性变换；并且它是根据向量v和w来定义的；

2）然后我通过两种不同方式来考虑这个变换的对偶向量；

<img src="./img/ch0802_19.png" width="60%"/>

3）应用这个变换，和与这个对偶向量点乘等价；



<img src="./img/ch0802_20.png" width="60%"/>

4）一方面，计算方法引导你使用下面这种技巧：

- 在矩阵第一列中插入$\hat{i},  \hat{j},  \hat{k}$，然后计算行列式；

<img src="./img/ch0802_21.png" width="60%"/>

5）另一个方面：但是从几何角度思考，我们可以推断出这个对偶向量必然与v和w垂直，并且其长度与这两个向量张成的平行四边形的面积相同；

【小结】上述这两种方法给出了同一个变换的对偶向量，因为这两个向量必然相同； 

<img src="./img/ch0802_22.png" width="60%"/>
...
- 把这些数看做拉伸或压缩向量的标量；
  ...
  - 这2个经过缩放向量的和就是坐标所要描述的向量；

<img src="./img/ch09_01.png" width="50%"/>

2）可以把这2个特殊向量(i,j)看做是封装坐标系的隐含假设；

- 第1个数字表示向右运动；
- 第2个数字表示向上运动；
- 单位长度；

3）这2个向量(i,j)是标量缩放的对象；

<img src="./img/ch09_02.png" width="50%"/>
...
坐标[1]（coordinates）是用来描述或标示某点在空间中绝对位置的有序数组[2][3]。坐标系（coordinate system）是一特定的位置注记系统[4]（参考系[5]），用以确定某点或某目标的空间位置；其定义为：对于一个 n 维系统，能够使每一个点（目标）和一组 n 个标量（有序数组）构成一一对应的系统[6][7]。

<font color=red>坐标系定义：n维空间中每个点与n个标量（有序数组）构成对应关系的系统；坐标：描述空间中某个点的绝对位置的有序数组；</font>



2）下面讨论使用另一组基向量的思想；

- 如使用基向量$b_1, b_2$， 分别指向右上方，左上方；

<img src="./img/ch09_03.png" width="40%"/>

3）向量 (5/3, 1/3) 在上述坐标系的表示如下：

<img src="./img/ch09_04.png" width="50%" />

无论你用什么坐标来描述这个向量，都是将第1个坐标乘以$b_1$，第2个坐标乘以$b_2$，然后将结果相加；

<font color=red>最终得到的向量会与你我认为的相同坐标的向量完全不同</font>；

4）例：基向量$b_1=\begin{bmatrix}2 \\ 1\end{bmatrix}, b_2=\begin{bmatrix}-1 \\ 1\end{bmatrix}$。

<img src="./img/ch09_05.png" width="50%" />

这与基向量$\hat{i}=\begin{bmatrix}1 \\ 0\end{bmatrix}, \hat{j}=\begin{bmatrix} 0 \\ 1\end{bmatrix}$ 定义的坐标系不是同一个坐标系，可以理解为是不同语言或在不同坐标系来描述同一个向量。（<font color=red>不同语言指的是不同坐标系</font>） 
...
1）在基向量$b_1=\begin{bmatrix}2 \\ 1\end{bmatrix}, b_2=\begin{bmatrix}-1 \\ 1\end{bmatrix}$定义的坐标系中，使用坐标(-1, 2)来描述一个向量，那么这个向量的坐标（或物理坐标或标准坐标）计算如下。
$$
(-1)\begin{bmatrix} 2 \\ 1 \end{bmatrix} + 2\begin{bmatrix} -1 \\ 1 \end{bmatrix}=\begin{bmatrix} -4 \\ 1 \end{bmatrix}
$$
这里使用某个向量的特定坐标(-1,2)与它的基向量数乘，然后将结果相加，看起来有些眼熟；<font color=red>这就是矩阵向量乘法</font>；
$$
\begin{bmatrix} 2 & -1 \\ 1 & 1 \end{bmatrix}  \begin{bmatrix} -1 \\ 2 \end{bmatrix}=(-1)\begin{bmatrix} 2 \\ 1 \end{bmatrix} + 2\begin{bmatrix} -1 \\ 1 \end{bmatrix}=\begin{bmatrix} -4 \\ 1 \end{bmatrix}
$$
这个矩阵的列就是某坐标系（非标准坐标系）的基向量；

2）实际上，一旦你将矩阵向量乘法理解为应用一个特定的线性变换，就会有一个非常直观的方法来理解这里发生的事情；

- 一个矩阵的列为詹尼佛坐标系的基向量（非标准坐标系的基向量），这个矩阵可以看做一个线性变换，它将标准坐标系中的基向量$\hat{i}=\begin{bmatrix}1 \\ 0\end{bmatrix},  \hat{j}=\begin{bmatrix}0 \\ 1\end{bmatrix}$， 变换为詹尼佛坐标系的基向量，也就是詹尼佛坐标系的"(1,0),(0,1)"。

3）从几何上说，这个矩阵是将我们的标准网格变换为詹尼佛的网格；但是从数值上说，这是用詹尼佛的语言描述转换为用我们的语言描述（<font color=red>即把詹尼佛坐标系转换为标准坐标系</font>）；

<img src="./img/ch09_10.png" width="60%"/>

也就是说，在我们的坐标系中具有相同坐标的向量，变换为它真正想要的向量；
...
- 实践中，尤其是在超过二维空间中做研究时，你可以用计算机来计算矩阵的逆；

2）在这里，<font color=red>对于以詹尼佛的基向量作为列的基变换矩阵</font>，其逆矩阵的两列为(1/3, -1/3), (1/3, 2/3)；如果想知道标准坐标系中坐标$\begin{bmatrix}3 \\ 2\end{bmatrix}$在詹尼佛坐标系的坐标如何表示，我们用这个詹尼佛基变换矩阵的逆乘以向量$\begin{bmatrix}3 \\ 2\end{bmatrix}$，结果为(5/3, 1/3)。



<img src="./img/ch09_11.png" width="60%"/>

总结：以上就是如何在坐标系之间对单个向量的描述进行相互转化。
...
1）一个矩阵的列代表的是詹尼佛的基向量，却是用我们的坐标（即标准坐标系的坐标）来描述；

- <font color=red>对于一个向量，却是用它的语言（如詹尼佛基向量定义的坐标系）描述转化为用我们的语言（标准坐标系）来描述</font>；
...
3）不过，向量并不是唯一用坐标来表示的东西；

- <font color=red>重要的是，接下来的内容里，你需要熟悉，矩阵代表线性变换；以及矩阵乘积对应于线性变换复合这两点</font>。

<img src="./img/ch09_23.png" width="70%"/>
...
1）考虑某个线性变换，比如逆时针旋转90度；

- 你我用矩阵代表它的时候，我们是在跟踪$\hat{i}, \hat{j}$的去向；$\hat{i}$变换后处于坐标(0,1)，而$\hat{j}$变换后处于(-1,0)；
- 这些坐标称为了矩阵的列；

2）但是这种表示与我们选择的基向量相关；

- 因为我们跟踪的是$\hat{i}, \hat{j}$，并且是在我们自己的坐标系中记录它们的去向；

<img src="./img/ch09_24.png" width="60%" />

3）詹尼佛如何描述90度的逆时针旋转？

- 你可能会尝试只将旋转矩阵的列转换为用詹尼佛的语言（<font color=red>詹尼佛坐标系</font>）来描述；
- 但是并不尽然，这些列代表的是$\hat{i}, \hat{j}$的去向；
- 但是詹尼佛想要的矩阵需要代表它的基向量的去向，并且使用它的语言(坐标系)来描述；

<img src="./img/ch09_25.png" width="60%" />
...
1）<font color=red>从詹尼佛语言（詹尼佛坐标系）</font>描述的任一向量出发；

- 首先，我们不用它的语言描述这一过程；
- 而是用基变换矩阵转化为用我们的语言描述；
- 这个矩阵的列代表的是用我们的语言（标准坐标系）描述的它的基向量；
- 此时给出的是同样一个向量，不过是用我们的语言来描述的；

<img src="./img/ch09_26.png" width="60%" />

<font color=red>向量(-1,2)是詹尼佛坐标系中的坐标</font>；
$$
...
4）因为我们能够对詹尼佛语言描述的任一向量做同样的事情：

- 应用基变换；
- 然后应用线性变换；
- 最后应用基变换的逆；

<img src="./img/ch09_29.png" width="70%"  />
...
- 如果你去计算的话，这3个矩阵的乘积的列为(1/3, 5/3)和(-2/3, -1/3) ；
...
1）每当你看到这样一个表达式，$A^{-1}MA$。这暗示着数学上的转移作用。 

- 中间的矩阵代表一种你所见的变换；
- 而外侧两个矩阵代表着转移作用（其他坐标系的基向量组成的矩阵），也就是视角上的转化；
- 矩阵乘积仍然代表着同一个变换，只不过是从其他人的视角来看的；

2）下期预告：

- 对于那些想知道为什么关注坐标系变换的人来说，下期关于特征值和特征向量的视频会给出一个非常重要的实例；
- 下期视频：特征向量与特征值
...
- 大部分向量在变换中都离开了该向量所张成的空间；
- 如果某个向量正好落在这条直线上，即没有离开其张成的空间，感觉更像是巧合；
- <font color=red>不过某些特殊向量的确留在它们张成的空间里；意味着矩阵对它的作用仅仅是拉伸或压缩而已，如同一个标量</font>；

3）例：特殊向量被线性变换后留在该向量所张成的空间里；

- 在这个例子中， $\hat{i}$就是这样一个特殊向量；
- $\hat{i}$张成的空间是x轴，而且从矩阵的第1列可以看出，$\hat{i}$变成了原来的3倍，仍然在x轴上；
- 此外，x轴上的任何其他向量都被拉升了3倍；因此也就留在它们张成的空间里；

4）例：有一个不起眼的向量(-1,1)，它在变换中也留在自己张成的空间里；最终被拉升为原来的2倍；

<img src="./img/ch10_01.png" width="60%"/>

标准坐标系中基向量即物理坐标(1,0)与(0,1)在基向量(3,0),(1,2)张成的坐标系被表示如下：。
$$
...
即(3,0), (1,2)就是新坐标系的基向量，如上图的绿色箭头与红色箭头所示；

4.1）例：对于物理坐标(-1,1) [物理坐标指标准坐标]，其在新坐标系的坐标表示如下。
$$
...
这个对角线所属的可选多个方向表示为如下向量。（<font color=red>下图中张成空间的对角线有2个，如黄色向量，还有一个是红色+绿色向量相加的向量</font>）
$$
...
<font color=red>特征向量定义：在变换后被拉伸n倍的向量</font>；

<img src="./img/ch10_06.png" width="60%"/>

每个特征向量都有一个所属的值，被称为特征值； 即衡量特征向量在变换中拉伸或压缩比例的因子。

<font color=red>特征值定义：衡量特征向量在变换中拉伸或压缩比例；</font>

【例】你可以有一个属于特征值-1/2的特征向量；意味着这个向量被反向，并被压缩为原来的1/2。但重点是，它停留在它张成的直线上，并没有发生旋转；
...
- 如果你能够找到这个旋转的特征向量，也就是留在它所张成的向量，那么你找到的就是<font color=red>旋转轴</font>；
- 而且把一个三维旋转考虑为看做是围绕某个轴旋转一定角度，要比考虑相应3X3矩阵直观得多；
- 顺带说下，这种情况下，相应的特征值必须为1，因为旋转并不缩放任何一个向量；所以向量长度保持不变；

2）这种规律在线性代数中经常出现；

- 对于任意矩阵描述的线性变换，你可以通过把矩阵的列看做是变换后的基向量来理解它；
- 但是，理解线性变换作用的关键往往较少依赖你的特定坐标系；
- <font color=red>更好的方法是计算它的特征向量与特征值</font>；
...
<img src="./img/ch10_07.png" width="60%"/>

2）对上述等式的理解：计算 $A\vec{v}=\lambda\vec{v}$，实际上是在求解使得这个等式成立的向量$\vec{v}$和数$\lambda$。

- 等号左边：代表矩阵向量乘积
- 等号右边：代表向量数乘；
- 变换矩阵A的作用效果：是将任意向量乘以$\lambda$

<img src="./img/ch10_08.png" width="60%" />

3）这个矩阵A的列：代表变换后的基向量； 

而每个基向量仅仅与$\lambda$相乘；所以这个矩阵的对角元素均为$\lambda$，其余位置全部是0；

<img src="./img/ch10_09.png" width="60%"/>

4）通常写法：提出因子$\lambda$，写做$\lambda$乘以$I$；这里的$I$就是单位矩阵，对角元全部为1；这里的$I$就是单位矩阵，对角元均为1；

<img src="./img/ch10_10.png" width="60%" />

## 【10.3】特征向量方程

1）现在两侧都是矩阵向量乘积的形式，我们能够把等号右侧的东西移到左侧，然后提出因子$\vec{v}$。得到一个新矩阵即矩阵$A$减去$\lambda$乘以单位阵$I$。

我们需要寻找向量$\vec{v}$使得这个新矩阵与$\vec{v}$相乘结果为零向量，如下。
$$
(A-\lambda I)\vec{v}=0
$$
<img src="./img/ch10_11.png" width="60%"/>

 2）如果v本身就是零向量，则这个等式恒成立；但我们要计算的是非零的特征向量； 

- 也就是说：<font color=red>当且仅当矩阵代表的变换将空间压缩到更低维度时，才会存在一个非零向量，使得矩阵和它的乘积为零向量</font> ，即$(A-\lambda I)\vec{v}=0$；
- 而空间压缩：对应的就是矩阵的行列式为零，即$det(A-\lambda I)=0$；
...
【例】矩阵$A=\begin{bmatrix}2 & 2 \\ 1 & 3\end{bmatrix}$, 则$A-\lambda I$计算如下：

- 考虑每个对角元素都减去某个变量$\lambda$，当我们逐步调整$\lambda$ 的值，矩阵本身发生改变，其行列式也在发生改变；
- 我们的目标在于找到一个$\lambda$，使得这个行列式为零；

<img src="./img/ch10_12.png" width="60%"/>

找到一个$\lambda$，使得这个行列式为零的计算公式，如下($\lambda=1$恰到好处，即 $\lambda=1是特征值$ )。 

<img src="./img/ch10_13.png" width="50%"/>
...
1）当$\lambda=1$时，A减去$\lambda$ 乘以单位阵将空间压缩到一条直线上。这意味这存在一个非零向量$\vec{v}$，使得A减去$\lambda$ 乘以单位阵的结果乘以$\vec{v}$ 等于零向量；
$$
(A-\lambda I)\vec{v}=\vec{0}
$$
记住，我们关注上式，是因为它意味着 $A\vec{v}=(\lambda I) \vec{v}$，即 $A\vec{v}=\lambda\vec{v}$ 。 

<font color=red>也就是说，向量$\vec{v}$是A的一个特征向量，在变换中向量 $\vec{v}$ 停留在它张成的空间里  ；</font> 在这个例子中，$\vec{v}$对应的特征值是1，所以它实际上保持不变；
...
1）步骤1：矩阵$A=\begin{bmatrix}3 &1 \\ 0 &2 \end{bmatrix}$，为了求解特征值$\lambda$，将矩阵的对角元减去$\lambda$，然后计算行列式；

<img src="./img/ch10_15.png" width="50%"/>

2）步骤2：我们就得到了关于$\lambda$的二次多项式$(3-\lambda)(2-\lambda)$。因为只有当这个行列式为零时，$\lambda$才会是特征值。

- 你就能够推断出，所有可能的特征值是$\lambda$等于2 和 $\lambda$等于3。

3）步骤3：为了计算某个特征值的特征向量，如$\lambda$等于2，把$\lambda$的值代入矩阵中，然后求解在这个对角线变换的矩阵变换后成为零的向量。

<img src="./img/ch10_16.png" width="50%"/>

4）步骤4：如果进行计算，如同求解其他线性方程组一样。你会发现所有的解全部都落在由向量(-1, 1)张成的对角线上。 

- 与之对应的，就是原始的矩阵$\begin{bmatrix} 3 &1 \\ 0 &2\end{bmatrix}$ ，将这些向量（方向与向量(-1,1)平行）拉伸为原来的2倍。
...
【例1】考虑逆时针90度的旋转，它并没有特征向量，因为每一个向量都发生了旋转并离开了其张成的空间；

2）如果你真的尝试取计算它的特征值，注意一下会发生什么呢？

- 矩阵列为$\begin{bmatrix}0 \\ 1\end{bmatrix}$和$\begin{bmatrix}-1 &0\end{bmatrix}$。对角元减去$\lambda$后，然后寻找行列式为0的情形。得到 $\lambda^{2}+1=0$（没有实数解表明它没有特征向量）。

<img src="./img/ch10_17.png" width="50%"/>
...
1）剪切变换定义：它保持$\hat{i}$不变，把$\hat{j}$向右移动一个单位，所以矩阵列为$\begin{bmatrix}1 &1 \\ 0 &1 \end{bmatrix}$

所有x轴的向量都属于特征值1的特征向量，因为它们都保持不变。

实际上，这些就是所有的特征向量；

<img src="./img/ch10_18.png" width="50%"/>

2）当你将对角元减去$\lambda$，然后计算行列式，你得到的是$(1-\lambda)^2$。

- 上述多项式$(1-\lambda)^2=0$的解是$\lambda=1$。
- 这与几何上得到的所有特征向量均属于特征值1的结果一致。

<img src="./img/ch10_19.png" width="50%"/>



3）注意：可能会出现只有一个特征值，但特征向量不止在一条直线上的情况；

【例】把所有向量变为2倍的变换矩阵；

- 唯一的特征值是2，但是平面内每一个向量都属于这个特征值的特征向量；

<img src="./img/ch10_21.png" width="60%"/>
...
<img src="./img/ch10_22.png" width="50%"/>

<img src="./img/ch10_23.png" width="50%"/>

2）<font color=red>如果我们的特征向量恰好是基向量，会发生什么</font>？

【例】如$\hat{i}$变为原来的-1倍，$\hat{j}$变为原来的2倍；将变换后基向量的新坐标作为矩阵的列，矩阵=$\begin{bmatrix}-1 &0 \\ 0 &2\end{bmatrix}$；

- 注意，它们的倍数是-1和2 ，也就是$\hat{i}和\hat{j}$ 所属的特征值。（位于矩阵的对角线上，而其他元素均为0）

<img src="./img/ch10_24.png" width="50%"/>
...
1）<font color=red>对角阵：除开对角元以外其他元素均为0的矩阵被称为对角阵</font>；

<img src="./img/ch10_25.png" width="50%"/>

<font color=red>解读它的方法是：所有基向量都是特征向量，矩阵的对角元是他们所属的特征值</font>；
...
<img src="./img/ch10_26.png" width="50%"/>

- <font color=red>相比之下，尝试计算一个非对角矩阵的100次幂，非常难计算，这就是一场噩梦</font>；

4）当然，对于基向量同时也是特征向量的情况，你可能不像它那么幸运；

- 但如果你的变换有许多特征向量，就像视频开始时的例子一样，多到你能够选出一个张成全空间的集合；
- 那么你就能够变换你的坐标系，使得这些特征向量就是基向量；

<img src="./img/ch10_27.png" width="50%"/>
...
1）如何在另一个坐标系中表达当前坐标系所描述的变换；

- 步骤1：取出你想用做新基的向量坐标；在这里指的是2个特征向量；
- 步骤2：然后将坐标作为矩阵的列，这个矩阵就是基变换矩阵$\begin{bmatrix} 1 &-1 \\ 0 &1\end{bmatrix}$；
- 步骤3：在右侧写下基变换矩阵，在左侧写下基变换矩阵的逆； <font color=red>当你将原始的变换夹在两个矩阵中间时，所得的矩阵代表的是同一个变换</font>；
  - 不过是从新基向量所构成的坐标系的角度来看的；

<img src="./img/ch10_28.png" width="50%"/>

2）<font color=red>用特征向量来完成这件事情的意义在于：这个新矩阵必然是对角的，并且对角元为对应的特征值</font>；

- 这是因为，它所处的坐标系的基向量在变换中只进行了缩放；
- <font color=red>新矩阵是对角阵，用于方便计算原始变换矩阵的n次幂，这就是特征向量作为基向量的作用(非常重要)</font>； 

<img src="./img/ch10_41.png" width="50%"/>
...
1）特征基定义：<font color=red>一组基向量（同样是特征向量）所构成的集合，被称为一组特征基，这也非常合理</font>；如基向量$\begin{bmatrix}1 \\ 0\end{bmatrix}$， $\begin{bmatrix}-1 \\ 1\end{bmatrix}$。 

<img src="./img/ch10_30.png" width="50%"/>



2）所以说，如果你要计算这个矩阵的100次幂，

- 一种更容易的做法是：先变换到特征基，在那个坐标系中计算100次幂，然后转换回标准坐标系；
...
4）对于那些愿意动手计算的人，有一道练习题：帮助你理解矩阵幂次计算的实际过程；以及它所能带来的惊人结论；

<img src="./img/ch10_31.png" width="60%"/>
...
3）你可能已经知道它的结果了，但真的要把它叙述出啦还是有点拗口。

- 这个新函数(f+g)在任意一点处的值，比如说在-4处的值，就是f和g函数在-4处的函数的值相加的和。

<img src="./img/ch11_02.png" width="50%"/>
...
更一般的说，这个函数在任意一点处x的值 (f+g)(x)等于f(x)+g(x)；

4）这与向量坐标相加非常相似；只不过在某种程度上说，它由无数个坐标相加；

<img src="./img/ch11_03.png" width="50%"/>
...
1）函数与一个实数相乘，也有一个合理的解释；

- 只是把输出的值与那个数相乘，这再次和向量对应坐标数乘类似；只是感觉有无数个坐标相乘；

<img src="./img/ch11_05.png" width="50%" />

2）因为对向量所能进行的操作不过是相加和数乘两种；

- 所以，最初以空间中的箭头为背景考虑的线性代数的合理概念与解决问题的手段，应该能够原封不动的被我们取出来；然后应用到函数上；

【例1】函数的线性变换有一个合理的解释：

- 这个变换接受一个函数，并把它变成另一个函数；

<img src="./img/ch11_06.png" width="50%"/>

【例2】微积分中的导数：它将一个函数变换到另一个函数；

<img src="./img/ch11_07.png" width="50%"/>

补充：关于这一点，有时你听到的是算子，而不是变换，但它们的意思是相同的；

<br>

---

### 【11.2.3】<font color=red>问题：一个函数变换是线性的？是什么意思</font>？

1）线性的严格定义：是相对抽象而符号繁重的；

- 但是，抽象性带来的好处是我们能够得到一般性的结论；
- 它不仅适用于箭头，也适用于函数；

2）<font color=red>满足以下两个性质的变换是线性的：这两条性质被称为可加性和成比例，然后对它们的和应用变换</font>；

<img src="./img/ch11_08.png" width="60%"/>

可加性：如果你把两个向量v和w相加，然后对它们的和应用变换，得到的结果和将变换后的v与变换后的w相加的结果一致；

<img src="./img/ch11_09.png" width="60%"/>

成比例：将一个向量v与某个数相乘，然后应用变换，得到的结果和变换后的v与这个数相乘一致；

<img src="./img/ch11_10.png" width="60%"/>

3）你经常会听到的一种描述方法是：<font color=red>线性变换保持向量加法运算和数乘运算</font>；

4）我在前几期视频中讨论的网格线保持平行且等距分布的概念：只是这两条性质在二维空间中这一特殊情况下的体现；

5）这两条性质的一个最重要推论：一个线性变换可以通过它对基向量的作用来完全描述，这使得矩阵向量乘法成为可能。

- 因为任意向量，都能表达为基向量以某种方式进行线性组合；
- 所以求一个向量变换后的结果，实际上就是计算变换后的基向量以相同方式进行线性组合的结果；

<img src="./img/ch11_11.png" width="50%"/>

- 稍后你就会明白：跟箭头一样，这一点对函数来说同样正确；

6）比如说：学微积分的同学经常会用到一个事实：求导具有可加性和成比例性（<font color=red>求导具有线性运算性质</font>）；

<img src="./img/ch11_12.png" width="60%"/>

如果把两个函数相加，然后求导数，等同于先求两个函数的导数，然后把结果相加；

<img src="./img/ch11_13.png" width="50%"/>

类似地，如果你将函数与数相乘，然后求导数，等同于先求导数，然后把结果与数相乘；

<img src="./img/ch11_14.png" width="50%"/>
...
1）为了真正掌握这里的类比关系，我们来看看用矩阵描述求导是什么样子的？

- 这可能有点棘手，因为函数空间倾向于无穷维；
- 虽然空间中的每一个多项式都只有有限项，但整个空间应该包含任意高次的多项式；

<img src="./img/ch11_15.png" width="50%"/>

2）给这个空间赋予坐标的含义，这需要选取一个基；

- 因为多项式已经是数乘x的不同次幂再做加和的形式；
- 所以我们很自然就取x的不同次幂作为基函数；
- 换句话说，第一个基函数就是一个常函数，即  $b_0(x)= 1 $

<img src="./img/ch11_16.png" width="60%"/>

第2个基函数就是$b_1(x)= x $，然后是$b_2(x)=x^2$， $b_3(x)=x^3$， 以此类推。

3）基函数在这里起到的作用： 和$\hat{i}, \hat{j}, \hat{k}$在向量的世界中起到的作用类似；

<img src="./img/ch11_17.png" width="60%"/>

因为多项式的次数可以任意高，所以这个基函数集也是无穷大的；

不过没有关系，这只是说明我们把多项式当做向量处理时，它们会有无穷多个坐标；

<img src="./img/ch11_18.png" width="60%"/>

【例】多项式$x^2+3x+5$，用坐标来表示的话是(1,3,5)，然后跟上无穷多个0；

你可以这样来理解它，5乘以第一个基函数，加上3乘以第二个基函数，再加上1乘以第三个基函数；在此之后，其他基函数不再出现；

<img src="./img/ch11_19.png" width="50%"/>

总的来说，因为每一个多项式都只有有限项，所以他的坐标就是有限长的一串数字，再跟上无限长的一串零。

4）在这个坐标系中，求导是用一个无限阶矩阵描述的；其中绝大部分是零，不过次对角线上按序排列着正整数；

<img src="./img/ch11_21.png" width="50%" />

5）我们稍后讨论如何求解这个矩阵：

6）不过熟悉它的最好方法是：看看它的作用过程；

- 取出这个多项式$x^3+5x^2+4x+5$的坐标，然后把坐标放在矩阵的右侧；
- 对结果的第1个坐标有贡献的只有1乘以4这一项，也就是说结果的常数项是4，这对应于$4x$的导数是常数4；
- 对矩阵向量乘积的第2个坐标有贡献的只有2乘以5这一项，也就是说，结果中x的系数是10；这对应于$5x^2$的导数是$10x$；
- 矩阵向量乘积的第3个坐标是3乘以1；这对应于$x^3$的导数是$3x^2$；
- 在此之后的坐标都是零；

<img src="./img/ch11_22.png" width="50%"/>

求导满足线性性质使得这一过程称为可能；

而且对那些暂停思考的人来说，你可以用下面这种方式构建这个矩阵：求每个基函数的导数，然后把结果放在对应列。

7）出乎我们意料的是：矩阵向量乘法原本与求导不相干；但它们两个其实是一家人；

<img src="./img/ch11_23.png" width="50%"/>
...
1）本视频中，我讨论的大部分关于向量（空间中带箭头）的概念：

- 比如说点积或特征向量；
- 在函数的世界中有直接类比；不过有时候它们的名称不同，比如说内积或特征函数；
- <img src="./img/ch11_24.png" width="50%" />
...
1）我想在这里指出的是，数学中有很多类似向量的事物；

只要你处理的对象集具有合理的数乘和相加的概念，不管是空间中的箭头，一组数，函数的集合，还是你定义的其他奇怪东西的集合；

<img src="./img/ch11_26.png" width="50%"/>

线性代数中所有的关于向量，线性变换和其他概念都应该适用它；

<img src="./img/ch11_28a.png" width="50%"/>

2）现在想象一下：你是一名发展线性代数理论的数学家，你想让你所做的所有定义和发现，不只对一个特殊情况适用，对其他类似向量的事物也具有普适性；

<img src="./img/ch11_29.png" width="50%"/>

这些类似向量的事物，如箭头，一组数，函数等，它们构成的向量集合被成为向量空间；

<img src="./img/ch11_30.png" width="50%"/>
...
1）你作为一个数学家，很可能想说，大家听好了，我可不想考虑你们构思出来的乱七八糟的向量空间。

所以你需要的是建立一系列向量加法和数乘必须遵守的规则；

<img src="./img/ch11_31.png" width="50%"/>

2）这些规则被称为公里；

<img src="./img/ch11_32.png" width="50%"/>

3）在线性代数的现代理论中，如果要让所有已经建立好的理论和概念都适用于一个向量空间，那么它必须满足上述8个规则。

- 不过这8条公理就是一个清单，以保证向量加法与数乘的概念确实是你想要的那样；

4）这些公理并非基础的自然法则，它们是一个媒介；

- 一边连接着你，也就是发现这些结论的数学家；
- 一边连接着其他人，也就是想要把这些结论用于新的向量空间的人；

5）假如说有人定义了一个奇怪的向量空间；

- 比如所有$\pi$ 生物的集合，并且定义了$\pi$ 生物的加法和数乘；这些公理就是一个清单；
- 你在其中应用线性代数的结论前，你需要验证它的这些定义是否满足要求；

6）作为一个数学家，你也不必再考虑人们定义的所有可能的奇怪的向量空间了；

- 你只需要根据这些公理证明你的结论；
- 只要这些人的定义满足这些公理，它们就可以顺利使用你的结论；即便你根本就没有想过它们的那种情况；
- 因此你往往会把你的所有结论抽象地表述出来；
- 也就说，仅仅根据这些公理表述，而不是集中于某一种特定向量上，像是空间中的箭头或函数等；

7）简而言之：就是为什么你阅读的每一本教科书，都会根据可加性和成比例来定义线性变换；

- 而不是用网格线保持平行且等距分布来定义；即便后者更加直观；
- 至少在我看来，后者对初学者更加有帮助；哪怕它只针对一种情况；

<img src="./img/ch11_33.png" width="50%"/>

8）所以对于向量是什么这个问题，数学家不会直接作答；

- <font color=red>在现代理论中，向量的形式并不重要；箭头，一组数，函数都不所谓；它可以是任何东西；只要向量相加和数乘能够满足上述8个规则即可</font>；

<img src="./img/ch11_34.png" width="50%"/>

9）就像是在问3是什么？

- 遇到具体情况时，它代表着三个东西的集合；
- 但在数学里，它被看做所有三个东西的集合的抽象概念；
- 从而让你用一个概念就能够推断出所有三个东西的集合；

<img src="./img/ch11_36.png" width="50%"/>

10）向量也是如此：他有很多种体现；

<img src="./img/ch11_37.png" width="50%" />

但是数学家把它抽象为向量空间这样一个抽象的无形概念；

10）<font color=red>看过这个系列的人都知道： 我还是推荐进行有形思考；即我觉得从一个具体形象的背景出发来梳理向量更好</font>；

- 比如二维空间中以原点为起点的箭头；
- 

<img src="./img/ch11_38.png" width="50%"/>

11）但是你学到了更多线性代数知识，知道了这些概念有更广泛的应用：

这就是教科书与课堂上更倾向于抽象表述的根本原因；（<font color=red>普适的代价是抽象</font>）

<img src="./img/ch11_39.png" width="50%"/>
