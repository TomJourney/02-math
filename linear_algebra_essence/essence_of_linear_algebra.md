...
1）$\vec{v}=\begin{bmatrix}0 \\ 0\\2\end{bmatrix}$ ，$\vec{w}=\begin{bmatrix} 0 \\ 2 \\ 2\end{bmatrix}$，显然v指向z轴正方向，w指向y轴正方向；它们围成的图形实际上是一个正方形；
...
1）这3个基向量的上述线性组合得到的向量是唯一一个与v和w组成平面垂直，长度为v和w围成的平行四边形面积，并且遵循有右手定则的向量；
